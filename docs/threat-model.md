...
| 3 | **Compromised bootstrap secret** | Device secrets are **only** exchanged for short-lived (5-min) JWTs and are never used to call business APIs. The issued token's `sub` is **bound to the VIN**, and vehicle-service rejects registration when the token subject's VIN ≠ the registering VIN. Registration is also gated on the vehicle having been spawned. | Per-device asymmetric attestation keys (no shared secret), hardware-backed keystore, certificate-based device identity, secret rotation/revocation. |
| 4 | **Unauthorized staff action** | Staff actions are authorized **server-side** by vehicle-service using explicit per-persona checks (`CanSpawnVehicle`, `CanAssignOwner`, …). The UI cannot bypass them; e.g. `manufacturing` cannot `assign_owner` and `sales_support` cannot `spawn`. Every denial is audited. | Real staff authentication (SSO/MFA), signed persona/role claims instead of a header, separation of duties, approval workflows. |
